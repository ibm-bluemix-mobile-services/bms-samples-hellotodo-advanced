...
3. After logging in to Bluemix using the command **cf login -a https://api.ng.bluemix.net** navigate to the NodeJS directory and run the **cf push your_Bluemix_app_name** command to deploy your application to Bluemix which will bind the custom Node.js code to the Mobile Client Access service instance and start the app.
